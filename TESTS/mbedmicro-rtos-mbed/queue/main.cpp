...
void thread_put_uint_msg(Queue<uint32_t, 1> *q)
{
    ...
    bool stat = q->try_put((uint32_t *) TEST_UINT_MSG);
...
void thread_get_uint_msg(Queue<uint32_t, 1> *q)
{
    ...
    TEST_ASSERT_TRUE(stat)
    TEST_ASSERT_EQUAL(TEST_UINT_MSG, v);
}
...
void test_pass_twice()
{
    ...
    stat = q.try_put((uint32_t *)TEST_UINT_MSG2);
    TEST_ASSERT_TRUE(stat);

    stat = q.try_get_for(Kernel::wait_for_u32_forever, &v);
    TEST_ASSERT_TRUE(stat);
    TEST_ASSERT_EQUAL(TEST_UINT_MSG2, v);
}
...
void test_put_full_no_timeout()
{
    ...
    stat = q.try_put((uint32_t *) TEST_UINT_MSG);
    TEST_ASSERT_FALSE(stat);
}
...
void test_put_full_timeout()
{
    ...
    bool stat = q.try_put_for(TEST_TIMEOUT, (uint32_t *) TEST_UINT_MSG);
    TEST_ASSERT_TRUE(stat);

    Timer timer;
    timer.start();

    stat = q.try_put_for(TEST_TIMEOUT, (uint32_t *) TEST_UINT_MSG);
    TEST_ASSERT_FALSE(stat);
    TEST_ASSERT_DURATION_WITHIN(TEST_TIMEOUT / 10, TEST_TIMEOUT, timer.elapsed_time());
}
...
void test_put_full_waitforever()
{
    ...
    Timer timer;
    timer.start();
    stat = q.try_put_for(Kernel::wait_for_u32_forever, (uint32_t *) TEST_UINT_MSG);
    TEST_ASSERT_TRUE(stat);
    TEST_ASSERT_DURATION_WITHIN(TEST_TIMEOUT / 10, TEST_TIMEOUT, timer.elapsed_time());
...
void test_msg_order()
{
    ...
    stat = q.try_put_for(TEST_TIMEOUT, (uint32_t *) TEST_UINT_MSG2);
    TEST_ASSERT_TRUE(stat);
...
void test_msg_prio()
{
    ...
    bool stat = q.try_put_for(TEST_TIMEOUT, (uint32_t *) TEST_UINT_MSG, 0);
    TEST_ASSERT_TRUE(stat);

    stat = q.try_put_for(TEST_TIMEOUT, (uint32_t *) TEST_UINT_MSG2, 1);
    TEST_ASSERT_TRUE(stat);
...
void test_queue_empty()
{
    ...
    q.try_put_for(TEST_TIMEOUT, (uint32_t *) TEST_UINT_MSG, 1);
