...
static mstd::unique_ptr<DNS_CACHE> dns_cache[MBED_CONF_NSAPI_DNS_CACHE_SIZE];
// Protects cache shared between blocking and asynchronous calls
static mstd::mutex dns_cache_mutex;
#endif

static uint16_t dns_message_id;
static mstd::unique_ptr<DNS_QUERY> dns_query_queue[DNS_QUERY_QUEUE_SIZE];
// Protects from several threads running asynchronous DNS
static mstd::recursive_mutex dns_mutex;
static SingletonPtr<call_in_callback_cb_t> dns_call_in;
static bool dns_timer_running = false;

static uint16_t nsapi_dns_next_id()
{
    if (++dns_message_id == 0) {
        dns_message_id = 1;
    }
...
// return true if a < b - ie a was used longer ago than b (or is empty)
/*static bool compare_dns_use(const mstd::unique_ptr<DNS_CACHE> &a, const mstd::unique_ptr<DNS_CACHE> &b)
{
    if (!a) {
        return b != nullptr;
    } else if (!b) {
...
static void nsapi_dns_cache_add(const char *host, nsapi_addr_t *address, uint32_t ttl)
{
    ...
    if (nsapi_dns_cache_find(host, address->version, nullptr) == NSAPI_ERROR_OK) {
        return;
    }

    mstd::scoped_lock<mstd::mutex> cache_lock(dns_cache_mutex);
    //mstd::unique_lock<mstd::mutex> cache_lock(dns_cache_mutex);

    // Find a free entry slot
    auto cache_entry_it = mstd::find(std::begin(dns_cache), std::end(dns_cache), nullptr);
    if (cache_entry_it != std::end(dns_cache)) {
        // Found a free entry slot, so allocate it
        DNS_CACHE *entry = new (std::nothrow) DNS_CACHE;
        if (!entry) {
            return;
        }
        cache_entry_it->reset(entry);
    } else {
        // Find the oldest entry, and we'll reuse it
        cache_entry_it = mstd::min_element(
                std::begin(dns_cache),
                std::end(dns_cache),
                [](const mstd::unique_ptr<DNS_CACHE> &a, const mstd::unique_ptr<DNS_CACHE> &b)
                {
                    return a->accessed < b->accessed;
                }
        );
    ...
    mstd::unique_ptr<DNS_CACHE> &cache_entry = *cache_entry_it;

    // Allocates in case entry is free, otherwise reuses
    /*if (!cache_entry) {
        cache_entry.reset(new (std::nothrow) DNS_CACHE);
    }*/

    cache_entry->address = *address;
    cache_entry->host.reset(new (std::nothrow) char[strlen(host) + 1]);
    if (!cache_entry->host) {
        cache_entry = nullptr;
        return;
    }
    strcpy(cache_entry->host.get(), host);
    uint64_t ms_count = rtos::Kernel::get_ms_count();
    cache_entry->expires = ms_count + (uint64_t) ttl * 1000;
    cache_entry->accessed = ms_count;
#endif
}
...
static nsapi_error_t nsapi_dns_cache_find(const char *host, nsapi_version_t version, nsapi_addr_t *address)
{
    ...
    for (auto &&cache_entry : dns_cache) {
        if (cache_entry) {
            uint64_t ms_count = rtos::Kernel::get_ms_count();
            // Checks all entries for expired entries
            if (ms_count > cache_entry->expires) {
                cache_entry = nullptr;
            } else if ((version == NSAPI_UNSPEC || version == cache_entry->address.version) &&
                       strcmp(cache_entry->host.get(), host) == 0) {
                if (address) {
...
static void nsapi_dns_cache_reset()
{
    ...
    mstd::fill(mstd::begin(dns_cache), mstd::end(dns_cache), nullptr);
#endif
}
...
// return pointer to the table entry, for when we want to modify the table
static mstd::unique_ptr<DNS_QUERY> *nsapi_dns_query_find_entry_by_id(int id)
{
#if 1
    auto it = mstd::find_if(
            mstd::begin(dns_query_queue),
            mstd::end(dns_query_queue),
            [id] (const mstd::unique_ptr<DNS_QUERY> &query_entry) {
                return query_entry && query_entry->id == id;
            }
    );
    return it != mstd::end(dns_query_queue) ? &*it : nullptr;
#else
    for (mstd::unique_ptr<DNS_QUERY> &query_entry : dns_query_queue) {
        if (query_entry && query_entry->id == id) {
            return &query_entry;
        }
    }
    return nullptr;
...
// return an the contents of a table entry, for when we're not planning to modify the table
static DNS_QUERY *nsapi_dns_query_find_by_id(int id)
{
    mstd::unique_ptr<DNS_QUERY> *query_entry = nsapi_dns_query_find_entry_by_id(id);
    return query_entry ? query_entry->get() : nullptr;
}
                                                      ...
                                                      nsapi_addr_t *addr, unsigned addr_count,  const char *interface_name, nsapi_version_t version)
{
    ...
    mstd::unique_ptr<uint8_t[]> packet(new (std::nothrow) uint8_t[DNS_BUFFER_SIZE]);
    if (!packet) {
    ...
    while (true) {
        ...
        // send the question
        int len = dns_append_question(packet.get(), 1, host, version);

        err = socket.sendto(dns_addr, packet.get(), len);
        ...
        // recv the response
        err = socket.recv(packet.get(), DNS_BUFFER_SIZE);
        ...
        const uint8_t *response = packet.get();
        uint32_t ttl;
...
{
    mstd::unique_ptr<nsapi_addr_t[]> addrs(new (std::nothrow) nsapi_addr_t[addr_count]);
    if (!addrs) {
        return NSAPI_ERROR_NO_MEMORY;
    }
    nsapi_size_or_error_t result = nsapi_dns_query_multiple(stack, host, addrs.get(), addr_count, interface_name, version);
...
{
    NetworkStack *nstack = nsapi_create_stack(stack);
    nsapi_size_or_error_t result = nsapi_dns_query_multiple(nstack, host, addr, 1, nullptr, version);
...
{
    nsapi_addr_t addr;
    nsapi_size_or_error_t result = nsapi_dns_query_multiple(stack, host, &addr, 1, nullptr, version);
...
nsapi_value_or_error_t nsapi_dns_query_async(NetworkStack *stack, const char *host,
                                             ...
                                             nsapi_version_t version)
{
    return nsapi_dns_query_multiple_async(stack, host, callback, 0, call_in_cb, nullptr, version);
}
                                                      ...
                                                      call_in_callback_cb_t call_in_cb, const char *interface_name, nsapi_version_t version)
{
    ...
    mstd::scoped_lock<mstd::recursive_mutex> dns_lock(dns_mutex);

    auto free_query_entry_it = mstd::find(mstd::begin(dns_query_queue), mstd::end(dns_query_queue), nullptr);
    if (free_query_entry_it == mstd::end(dns_query_queue)) {
        return NSAPI_ERROR_NO_MEMORY;
    }

    mstd::unique_ptr<DNS_QUERY> query(new (std::nothrow) DNS_QUERY);
    if (!query) {
        return NSAPI_ERROR_NO_MEMORY;
    }

    query->host.reset(new (std::nothrow) char[host_len + 1]);
    if (!query->host) {
        return NSAPI_ERROR_NO_MEMORY;
    }
    ...
    uint16_t unique_id = nsapi_dns_next_id();
    query->id = unique_id;

    int ongoing_queries = 0;
    for (auto &&query_entry : dns_query_queue) {
        if (query_entry) {
            if (!query->socket && query_entry->socket && query_entry->stack == query->stack) {
                query->socket = query_entry->socket;
                query->socket_cb_data = query_entry->socket_cb_data;
            }
            ongoing_queries++;
        }
    }
    ...
    *free_query_entry_it = std::move(query);

    if (!dns_timer_running) {
        if (nsapi_dns_call_in(call_in_cb, DNS_TIMER_TIMEOUT, mbed::callback(nsapi_dns_query_async_timeout)) != NSAPI_ERROR_OK) {
            *free_query_entry_it = nullptr;
            return NSAPI_ERROR_NO_MEMORY;
        }
...
{
    unsigned max_age = 0;
    DNS_QUERY *query = nullptr;

    // Trigger next query to start, find one that has been on queue longest
    for (auto &&query_entry : dns_query_queue) {
        if (query_entry) {
            if (query_entry->state == DNS_CREATED) {
                uint16_t age = dns_message_id - query_entry->id;
                if (age >= max_age) {
                    query = query_entry.get();
                    max_age = age;
                }
                // If some query is already ongoing do not trigger
            } else if (query_entry->state == DNS_INITIATED) {
                query = nullptr;
                break;
            }
        }
    ...
    if (query) {
        ...
        nsapi_dns_call_in(query->call_in_cb, 0, mbed::callback(nsapi_dns_query_async_create, reinterpret_cast<void *>(query->id)));
    }
}
...
static void nsapi_dns_query_async_timeout(void)
{
    ...
    for (mstd::unique_ptr<DNS_QUERY> &query_entry : dns_query_queue) {
        if (query_entry) {
            if (query_entry->state == DNS_CANCELLED) {
                // Delete cancelled
                nsapi_dns_query_async_delete(query_entry->id);
                nsapi_dns_query_async_initiate_next();
                continue;
            }

            if (query_entry->total_timeout > DNS_TIMER_TIMEOUT) {
                query_entry->total_timeout -= DNS_TIMER_TIMEOUT;
            } else {
                // If does not already have response, fails
                if (query_entry->status == NSAPI_ERROR_TIMEOUT) {
                    query_entry->socket_timeout = 0;
                    nsapi_dns_call_in(query_entry->call_in_cb, 0, mbed::callback(nsapi_dns_query_async_response, reinterpret_cast<void *>(query_entry->id)));
                }
            }

            if (query_entry->socket_timeout > 0) {
                if (query_entry->socket_timeout > DNS_TIMER_TIMEOUT) {
                    query_entry->socket_timeout -= DNS_TIMER_TIMEOUT;
                } else {
                    // Retries
                    query_entry->socket_timeout = 0;
                    nsapi_dns_call_in(query_entry->call_in_cb, 0,
                                      mbed::callback(nsapi_dns_query_async_send, reinterpret_cast<void *>(query_entry->id)));
                }
            }
...
nsapi_error_t nsapi_dns_query_async_cancel(int id)
{
    ...
    DNS_QUERY *query = nsapi_dns_query_find_by_id(id);
...
static void nsapi_dns_query_async_create(void *ptr)
{
    mstd::unique_lock<mstd::recursive_mutex> lock(dns_mutex);

    int id = reinterpret_cast<int>(ptr);
    ...
    for (auto &&query_entry : dns_query_queue) {
        if (query_entry && query_entry.get() != query) {
            if (!query->socket && query_entry->socket && query_entry->stack == query->stack) {
                query->socket = query_entry->socket;
                query->socket_cb_data = query_entry->socket_cb_data;
            }
    ...
    if (!query->socket) {
        UDPSocket *socket = new (std::nothrow) UDPSocket;
        if (!socket) {
            nsapi_dns_query_async_resp(query, NSAPI_ERROR_NO_MEMORY, nullptr);
            return;
        }
        ...
        if (err) {
            delete socket;
            nsapi_dns_query_async_resp(query, err, nullptr);
            return;
        }

        socket->set_timeout(0);
        if (query->interface_name != nullptr) {
    ...
    nsapi_dns_query_async_send(reinterpret_cast<void *>(query->id));
}

static nsapi_error_t nsapi_dns_query_async_delete(int unique_id)
{
    mstd::unique_ptr<DNS_QUERY> *query_entry_ptr = nsapi_dns_query_find_entry_by_id(unique_id);
    if (!query_entry_ptr) {
        return NSAPI_ERROR_PARAMETER;
    }

    DNS_QUERY *query = query_entry_ptr->get();

#if 1
    bool close_socket = mstd::none_of(
            mstd::begin(dns_query_queue),
            mstd::end(dns_query_queue),
            [query](const mstd::unique_ptr<DNS_QUERY> &e) {
                return e && e.get() != query && e->socket && e->stack == query->stack;
            }
    );
#else
    bool close_socket = true;
    for (const mstd::unique_ptr<DNS_QUERY> &e: dns_query_queue) {
        if (e && e.get() != query && e->socket &&
                e->stack == query->stack) {
            close_socket = false;
            break;
        }
...
static void nsapi_dns_query_async_resp(DNS_QUERY *query, nsapi_error_t status, SocketAddress *address)
{
    mstd::unique_lock<mstd::recursive_mutex> lock(dns_mutex, mstd::adopt_lock);

    NetworkStack::hostbyname_cb_t callback = query->callback;
    nsapi_dns_query_async_delete(query->id);
    nsapi_dns_query_async_initiate_next();
...
static void nsapi_dns_query_async_send(void *ptr)
{
    ...
    DNS_QUERY *query = nsapi_dns_query_find_by_id(unique_id);
    ...
    // send the question
    int len = dns_append_question(packet.get(), query->id, query->host.get(), query->version);
    ...
    while (true) {
        ...
        if (err != NSAPI_ERROR_OK) {
            lock.release();
            nsapi_dns_query_async_resp(query, NSAPI_ERROR_TIMEOUT, nullptr);
            return;
        }

        err = query->socket->sendto(dns_addr, packet.get(), len);
...
static void nsapi_dns_query_async_socket_callback_handle(NetworkStack *stack)
{
    ...
    auto it = mstd::find_if(
            mstd::begin(dns_query_queue),
            mstd::end(dns_query_queue),
            [stack](const mstd::unique_ptr<DNS_QUERY> &q) {
                return q && q->stack == stack;
            }
    );

    if (it == mstd::end(dns_query_queue)) {
        return;
    }

    UDPSocket *socket = (*it)->socket;
    ...
    while (true) {
        // recv the response
        nsapi_size_or_error_t size = socket->recv(packet.get(), DNS_BUFFER_SIZE);
        if (size < 0) {
            // terminate loop in case of error - not expecting any except "would block"
            break;
        }
        ...
        DNS_QUERY *query = it->get();
        if (query->state != DNS_INITIATED) {
            continue;
        }
        ...
        query->addrs.reset(new (std::nothrow) nsapi_addr_t[requested_count]);
        if (query->addrs) {
            int scan_result = dns_scan_response(packet.get(), id, &(query->ttl), query->addrs.get(), requested_count);
            if (scan_result < 0) {
                // Ignore invalid responses
                query->addrs = nullptr;
                query->count = 0;
            ...
            query->count = scan_result;
            query->status = NSAPI_ERROR_DNS_FAILURE; // Used in case failure (count=0), otherwise ok
        } else {
            query->status = NSAPI_ERROR_NO_MEMORY;
        }

        query->socket_timeout = 0;
        lock.release();
        nsapi_dns_call_in(query->call_in_cb, 0, mbed::callback(nsapi_dns_query_async_response, reinterpret_cast<void *>(query->id)));
    }
}
...
static void nsapi_dns_query_async_response(void *ptr)
{
    ...
    DNS_QUERY *query = nsapi_dns_query_find_by_id(unique_id);
    if (query && query->state == DNS_INITIATED) {
        mstd::unique_ptr<SocketAddress[]> addresses;
        nsapi_error_t status = query->status;

        if (query->count > 0) {
            addresses.reset(new (std::nothrow) SocketAddress[query->count]);
            if (addresses) {
                ...
                nsapi_dns_cache_add(query->host.get(), &(query->addrs[0]), query->ttl);
                ...
                status = NSAPI_ERROR_NO_MEMORY;
            }
        }

        lock.release();
        nsapi_dns_query_async_resp(query, status, addresses.get());
    }
}
